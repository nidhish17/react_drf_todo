        ...
        <Table>
            ...
            <Table.TBody>
                {tasks?.map((todo) => {
                    ...
                    return (
                        <Table.TR className={`${task_status ? "bg-green-700" : "odd:bg-zinc-600 even:bg-zinc-800"} border-t-yellow-500 border-t`}
                                  key={task_id}>
                            <Table.TD>{formatTime(task_created)}</Table.TD>
                            <Table.TD
                                className={`capitalize ${task_status && "line-through decoration-zinc-900 decoration-2"}`}>{task_title}</Table.TD>
                            ...
                            <Table.TD className="text-left">
                                <button
                                    onClick={() => openModal({
                                        ...
                                        title: <span
                                            className="flex justify-between items-center"><span>#{task_title}</span><span
                                            className="text-xs px-2 rounded-full bg-blue-500 py-0.5 text-white">{task_category_title}</span></span>
                                    })}
                                    className="cursor-pointer p-1.5 rounded bg-sky-500 hover:bg-sky-600 text-center">
                                    <FaInfo/></button>
                            </Table.TD>
                                ...
                                <div className="flex gap-x-1 justify-center">
                                    ...
                                    <Option>
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-x-2 cursor-pointer p-1.5 ${task_status ? "bg-green-500 hover:bg-green-600" : "bg-blue-500 hover:bg-blue-600"} rounded transition-all duration-200 text-sm font-medium`}>
                                            <SiTicktick />
                                            <p className="">complete</p>
                                        </button>
                                        <button onClick={() => handleDeleteBtn(task_id)} className="w-full flex items-center gap-x-2 cursor-pointer p-1.5 bg-red-500 hover:bg-red-700 rounded transition-all duration-200 text-sm font-medium">
                                            <MdDelete />
